...
func TestSignAndVerifyTransaction(t *testing.T) {
	...
	valid1, err1 := utils.XrpVerifySig(txHash1, sig1, &pk1.PublicKey)
	if err1 != nil {
		t.Fatalf("error verifying signature 1: %v", err1)
	}

	valid2, err2 := utils.XrpVerifySig(txHash2, sig2, &pk2.PublicKey)
	if err2 != nil {
		t.Fatalf("error verifying signature 2: %v", err2)
	}

	valid3, err3 := utils.XrpVerifySig(txHash3, sig3, &pk3.PublicKey)
	if err3 != nil {
