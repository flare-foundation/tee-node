...
func TestAbiEncodeDecodeFTDCProveResponse(t *testing.T) {
	...
	decodedResponseHeader, err := types.DecodeFTDCResponse(encoded)
	require.NoError(t, err)
...
func TestAbiEncodeDecodeFTDCProveRequest(t *testing.T) {
	...
	encoded, err := types.EncodeFTDCRequest(originalAttestationRequest)
	require.NoError(t, err)
	require.NotNil(t, encoded)

	// Decode the encoded data
	decodedAttestationRequest, err := types.DecodeFTDCRequest(encoded)
	require.NoError(t, err)
